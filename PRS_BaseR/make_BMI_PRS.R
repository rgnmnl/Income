...
h.snps <- subset(snps, Phenotype=="BMI")
index <- which(colnames(dat) %in% as.character(h.snps$SNP))
...
## Histogram of PRS
pdf("~/Data/Mike-Patrick-PRS/bmi-prs-hist.pdf")
hist(prs)
dev.off()

## Apply the PRS in the other 50% sample
y <- as.numeric(h.test$BMI)
x <- prs
mod <- lm(y~x)
...
colnames(dat.out) <- c("ID", "bmi", "bmi_PRS")
write.table(dat.out, "~/Data/Mike-Patrick-PRS/BMI-PRS.txt", col.names=TRUE, row.names=FALSE, quote=FALSE)
