...
h.snps <- subset(snps, Phenotype=="Depression")
...
## Missing data 
h.dat <- subset(h.dat, is.na(depression)==FALSE)


## Take a 50% sample from h.dat
samp <- sample(1:36700, size=18350, replace=FALSE)
samp2 <- setdiff(1:36700, samp)
...
beta <- NULL
p <- NULL
y <- as.numeric(h.build$depression)
...
## Histogram of PRS
pdf("~/Data/Mike-Patrick-PRS/depression-prs-hist.pdf")
hist(prs)
dev.off()

## Apply the PRS in the other 50% sample.
y <- as.numeric(h.test$depression)
x <- prs
mod <- glm(y~x, family="binomial")
...
colnames(dat.out) <- c("ID", "depression", "dep_PRS")
write.table(dat.out, "~/Data/Mike-Patrick-PRS/depression-PRS.txt", col.names=TRUE, row.names=FALSE, quote=FALSE)
