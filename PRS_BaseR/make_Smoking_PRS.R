...
### Get	rid of this SNP. It gives a completely messed up beta value. rs78239456_A
h.snps <- subset(snps, Phenotype=="Smoking" & SNP_1 != "rs78239456_A" & SNP_2 != "rs78239456_A")
...
### Missing phenotypes?
h.dat <- subset(h.dat, is.na(smk)==FALSE)

## Take a 50% sample from h.dat
samp <- sample(1:314929, size=157464, replace=FALSE)
samp2 <- setdiff(1:314929, samp)
...
## Histogram of PRS
pdf("~/Data/Mike-Patrick-PRS/smoking-prs-hist.pdf")
hist(prs)
dev.off()

## Apply the PRS in the other 50% sample.
y <- as.numeric(h.test$smk)
x <- prs
mod <- glm(y~x, family="binomial")
...
colnames(dat.out) <- c("ID", "smoking", "smk_PRS")
write.table(dat.out, "~/Data/Mike-Patrick-PRS/smoking-PRS.txt", col.names=TRUE, row.names=FALSE, quote=FALSE)
