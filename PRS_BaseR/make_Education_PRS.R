...
h.snps <- subset(snps, Phenotype=="Education")
...
### Missing phenotypes?
h.dat <- subset(h.dat, is.na(Age_Ed)==FALSE & Age_Ed > 0)

## Take a 50% sample from h.dat
samp <- sample(1:202622, size=101311, replace=FALSE)
samp2 <- setdiff(1:202622, samp)
...
## Histogram of PRS
pdf("~/Data/Mike-Patrick-PRS/education-prs-hist.pdf")
hist(prs)
dev.off()

## Apply the PRS in the other 50% sample.
y <- as.numeric(h.test$Age_Ed)
x <- prs
mod <- lm(y~x)
...
colnames(dat.out) <- c("ID", "Age_Ed", "Ed_PRS")
write.table(dat.out, "~/Data/Mike-Patrick-PRS/education-PRS.txt", col.names=TRUE, row.names=FALSE, quote=FALSE)
