##############################################################################
## Title: UKBB APOE Analysis
## Author: Regina Manansala
## Date Created: 10-February-2019
## Date Modified: 15-February-2019
##############################################################################
...
#Create caregiver variable
inc_nomiss_emp$caregiver <- ifelse(inc_nomiss_emp$empl_status == "Looking after home and/or family", 2, 1)
...
########## CREATE COVARIATE SUBSET FOR CAREGIVER ANALYSIS #################

caregiver_cov <- inc_analysis[,c(1,1,30,22,35,31,4:13)]
names(caregiver_cov)[1:6] <- c("#FID", "IID", "Caregiver", "Age", "Sex", "Location")

#Random Sample 80% of data for analysis/20% for replication
# install.packages("ISLR")
library(ISLR)
smp_siz = floor(0.8*nrow(caregiver_cov))
set.seed(123)
sample <- sample(seq_len(nrow(caregiver_cov)),size = smp_siz)
apoe_analysis <- caregiver_cov[sample,]
apoe_replicate <- caregiver_cov[-sample,]

#Create Phenotype Files
write.table(apoe_analysis, "APOE_CAREGIVER/apoe_analysis.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)
write.table(apoe_replicate, "APOE_CAREGIVER/apoe_replicate.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)

#Stratify by Gender
write.table(apoe_analysis[apoe_analysis$Sex == "Female", -5], "APOE_CAREGIVER/apoe_female.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)
write.table(apoe_analysis[apoe_analysis$Sex == "Male", -5], "APOE_CAREGIVER/apoe_male.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)

write.table(apoe_replicate[apoe_replicate$Sex == "Female", -5], "APOE_CAREGIVER/apoe_female_rep.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)
write.table(apoe_replicate[apoe_replicate$Sex == "Male", -5], "APOE_CAREGIVER/apoe_male_rep.txt", sep="\t", row.names=F, col.names=T, quote=FALSE)
...
plink2 \
    ...
    --pheno ~/Data/UKBB_PH/INC/APOE_CAREGIVER/apoe_analysis.txt \
    --pheno-name Caregiver \
    --covar ~/Data/UKBB_PH/INC/APOE_CAREGIVER/apoe_analysis.txt \
