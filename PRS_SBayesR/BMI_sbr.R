##############################################################################
## Title: UKBB BMI sBayesR Workflow
...
files <- list.files(path="BMI", pattern= ".fastGWA")

full <- data.table()
for(i in files){
	temp <- fread(paste0("BMI/", i))
...
write.table(full.qc, "bmi_qc_n.ma", col.names = T, row.names = F, sep = " ", quote = F)

for (i in seq(0.1, 0.8, 0.1))
{
  print(paste0("Doing ", i))
  full.qc.pval <- full.qc[which(full.qc$P < i), ] 
  write.table(full.qc.pval, paste0("gctb/BMI/bmi_qc_n_pval_",i, ".ma"),
...
#SBATCH --job-name=sbR_bmi

#printenv
#exit                                                                                                          

mkdir BMI_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}
mv bmi_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}.ma BMI_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}
...
~/Data/manansa2/gctb/gctb_2.02_Linux/gctb \
    --sbayes R \
    --mldm test_data/ukbEURu_hm3_sparse_mldm_list.txt \
    --gwas-summary gctb/BMI/BMI_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}/bmi_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}.ma \
    --out gctb/BMI/BMI_qc_n_pval_0.${SLURM_ARRAY_TASK_ID}/bmi_qc_n_pval_0.${SLURM_ARRAY_TASK_ID} \
    --chain-length 2000 --burn-in 1000 --out-freq 10
...
x <- read.table("BMI_qc_n/bmi_qc_n.snpRes", header = T)
x.2 <- x[,c("Name", "A1", "A1Effect")]
write.table(x.2, "BMI_qc_n/bmi_qc_n.params", row.names = F, sep = " ", quote = F)

for(i in 1:8){
	x <- read.table(paste0("BMI_qc_n_pval_0.", i, "/bmi_qc_n_pval_0.", i, ".snpRes"), header = T)
	x.2 <- x[,c("Name", "A1", "A1Effect")]
	write.table(x.2, paste0("BMI_qc_n_pval_0.", i, "/bmi_qc_n_pval_0.", i, ".params"), row.names = F, sep = " ", quote = F)
}
...
plink \
    ...
    --score gctb/BMI/BMI_qc_n/bmi_qc_n.params \
    --out gctb/BMI/BMI_qc_n/score/bmi_chr${SLURM_ARRAY_TASK_ID}
...
plink \
    ...
    --bfile ldsub_chr${SLURM_ARRAY_TASK_ID} \
    --exclude bim${SLURM_ARRAY_TASK_ID}.dups \
    --score gctb/BMI/BMI_qc_n_pval_0.1/bmi_qc_n_pval_0.1.params \
    --out gctb/BMI/BMI_qc_n_pval_0.1/score/bmi_chr${SLURM_ARRAY_TASK_ID}
...
ukb <- read.table("PRS/PLINK_BMI/bmi_pheno_test.txt")
...
for(k in 1:length(qc)){
	lky.chr1 <- read.table(paste0("BMI_qc_n", qc[k], "/score/bmi_chr1.profile"), header = T)
	lky.all  <- lky.chr1
	for (i in seq(2, 22)){
		print(paste("Doing", i))
		lky.chri <- read.table(paste0("BMI_qc_n", qc[k], "/score/bmi_chr", i, ".profile"), header = T)
		lky.all$SCORE <- lky.all$SCORE + lky.chri$SCORE
  	}
  	lky.ukb <- inner_join(ukb, lky.all, by = c("V1" = "FID"))
  	resLocke[k, 1] <- paste0("BMI_qc_n", qc[k])
	resLocke[k, 2] <- summary(lm(lky.ukb$V16 ~ lky.ukb$SCORE))$r.squared
	print(resLocke)
}
colnames(resLocke) <- c("QC", "Pred_R2")

write.table(resLocke, "bmi_results.txt", col.names = T, row.names = F, quote = F, sep = " ")
