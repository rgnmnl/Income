##############################################################################
## Title: UKBB Education (Numeric) sBayesR Workflow
## Version: 3
...
files <- list.files(path="EDU", pattern= "^edunum_chr(\\d|\\d\\d)\\.fastGWA$")
...
for (i in seq(0.1, 0.8, 0.1))
{
  ...
  write.table(full.qc.pval, paste0("gctb/EDU_num/edu_qc_n_pval_",i, ".ma"),
...
plink \
    ...
    --score gctb/EDU_num/EDU_qc_n/edu_qc_n.params \
    --out gctb/EDU_num/EDU_qc_n/score/edu_chr${SLURM_ARRAY_TASK_ID}
...
plink \
    ...
    --bfile /raid-05/SPH/pauer/UKBB/UKBB_G_Income/LD_PLINK/LD_BED/ldsub_chr${SLURM_ARRAY_TASK_ID} \
    --exclude /raid-05/SPH/pauer/UKBB/UKBB_G_Income/LD_PLINK/LD_BED/bim${SLURM_ARRAY_TASK_ID}.dups \
    --score gctb/EDU_num/EDU_qc_n_pval_0.1/edu_qc_n_pval_0.1.params \
    --out gctb/EDU_num/EDU_qc_n_pval_0.1/score/edu_chr${SLURM_ARRAY_TASK_ID}
...
ukb <- read.table("PRS/PLINK_EDU_num/edunum_pheno_test.txt")
...
qc <- c("", "_pval_0.1", "_pval_0.2", "_pval_0.3", "_pval_0.4", "_pval_0.5", "_pval_0.6", "_pval_0.7", "_pval_0.8")
	...
	lky.chr1 <- read.table(paste0("EDU_num_qc_n/score/edu_chr1.profile"), header = T)
	lky.all  <- lky.chr1
	for (i in seq(2, 22)){
		print(paste("Doing", i))
		lky.chri <- read.table(paste0("EDU_num_qc_n/score/edu_chr", i, ".profile"), header = T)
		lky.all$SCORE <- lky.all$SCORE + lky.chri$SCORE
  	}
  	lky.ukb <- inner_join(ukb, lky.all, by = c("V1" = "FID"))
  	resLocke[1, 1] <- "EDU_num_qc_n"
	resLocke[1, 2] <- summary(lm(lky.ukb$V16 ~ lky.ukb$SCORE))$r.squared
	print(resLocke)


for(k in 2:length(qc)){
...
write.table(resLocke, "edunum_results.txt", col.names = T, row.names = F, quote = F, sep = " ")
